                ...
                <Form @submit="handleUpdate" class="w-full" ref="formRef">
                    <div class="hx-card">
                        <div class="hx-card__header">
                            <h4 class="text-gray-600 text-xl">توضیخ محصول</h4>
                        </div>
                        <div class="hx-card__body">
                            ...
                            <hx-form-group>
                                <Field mode="passive" name="content" v-slot="{ field }" rules="required"
                                    label=" توضیحات">
                                    <TiptapEditor v-bind="field" v-model="form.content" :content="form.content"
                                        ref="tiptap">
                                    </TiptapEditor>
                                </Field>
                    ...
                    <div class="w-full flex items-center justify-start my-4">
                        <div class="flex items-center space-x-3 space-x-reverse">
                            ...
                            <hx-button variant="light"
                                :to="{ name: 'products reviews index', params: { id: product_id } }">
                                لغو
                            </hx-button>
                        </div>
                    </div>
...
const handleUpdate = async (values, { resetForm }) => {
    ...
    try {
        ...
        const { data } = await ApiService.post(
            `products/${product_id.value}/reviews`,
            formData
        );
        resetForm();
        loader.value = false;
