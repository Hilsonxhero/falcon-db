  ...
  <section class="mb-6">
    <div class="grid grid-cols-12 gap-4">
      <div class="col-span-12 lg:col-span-12">
        <HxDataTable @refresh="refresh = false" :refresh="refresh" :url="`products/${product_id}/reviews`"
          ...
          <template #left>
            <hx-button :to="{ name: 'products reviews create' }">ایجاد توضیحات</hx-button>
          </template>
          <template v-slot:cell-checkbox="{ row: review }">
            <div class="form-check form-check-sm form-check-custom form-check-solid">
              <input class="form-check-input" type="checkbox" :value="review.id" v-model="checkedData" />
            </div>
          </template>
          ...
          <template v-slot:cell-actions="{ row: review, index: index }">
            ...
            <hx-button variant="gray" size="sm" icon @click="handleDelete(review, index)">
