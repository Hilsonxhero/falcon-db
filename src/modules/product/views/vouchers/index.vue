    ...
    <section class="mb-6">
        <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12 lg:col-span-12">
                <HxDataTable @refresh="refresh = false" :refresh="refresh" :url="`vouchers`" :single-item-index="index"
                    search-placeholder="جستجوی  عنوان و.." :table-header="tableHeader"
                    :enable-items-per-page-dropdown="false" :on-current-change="true">

                    <template #left>
                        <hx-button :to="{ name: 'vouchers create' }"> کد تخفیف جدید</hx-button>
                    </template>


                    <template v-slot:cell-checkbox="{ row: voucher }">
                        <div class="form-check form-check-sm form-check-custom form-check-solid">
                            <input class="form-check-input" type="checkbox" :value="voucher.id" v-model="checkedData" />
                        </div>
                    </template>
                    ...
                    <template v-slot:cell-actions="{ row: voucher, index: index }">
                        ...
                        <hx-button variant="gray" size="sm" icon @click="handleDelete(voucher, index)">
