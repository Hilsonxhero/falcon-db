                ...
                <Form @submit="handleUpdate" class="w-full" ref="formRef">
                    <div class="hx-card">
                        <div class="hx-card__header">
                            <h4 class="text-gray-600 text-xl">ویرایش مقدار {{ form.name && form.name }}</h4>
                        </div>
                    ...
                    <div class="w-full flex items-center justify-start my-4">
                        <div class="flex items-center space-x-3 space-x-reverse">
                            ...
                            <hx-button variant="light" :to="{ name: 'variants values', params: { id: id } }">
                                لغو
                            </hx-button>
                        </div>
                    </div>
...
const handleUpdate = async (values, { resetForm }) => {
    ...
    try {
        const { data } = await ApiService.put(`variants/${variant.value}`, formData)
        resetForm();
