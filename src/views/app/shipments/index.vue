<template>
    <section class="mb-6">
        <HxDataTable url="shipments" :single-item-index="index" search-placeholder="جستجوی روش ارسال"
            ...
            <template v-slot:cell-checkbox="{ row: shipment }">
                <div class="form-check form-check-sm form-check-custom form-check-solid">
                    <input class="form-check-input" type="checkbox" :value="shipment.id" v-model="checkedData" />
                </div>
            </template>
            ...
            <template v-slot:cell-actions="{ row: shipment, index: index }">
                <hx-button variant="gray" size="sm" icon :to="{ name: 'shipments edit', params: { id: shipment.id } }">
                    <hx-icon icon="edit-alt"></hx-icon>
                </hx-button>
                <hx-button variant="gray" size="sm" icon @click="handleDelete(shipment, index)">
...
const handleDelete = (item: any, i: any) => {
    HxMessageBox.confirm(
        `آیا از حذف روش ارسال ${item.title} اطمینان دارید ؟!`,
        'پیام تایید',
        {
