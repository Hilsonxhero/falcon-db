...
const useSelect = (props: ExtractPropTypes<typeof SelectProps>, emit) => {
  ...
  const iconReverse = computed(() => nsSelectV2.is("reverse", expanded.value));
  ...
  const getLabel = (item: unknown) => {
    return isObject(item) ? get(item, props.label) : item;
  };
    ...
    if (props.multiple) {
      ...
      if (hasModelValue.value) {
        ...
        if (~selectedItemIndex) {
          ...
          if (states.selectedLabel === undefined)
            states.selectedLabel = options[selectedItemIndex];
